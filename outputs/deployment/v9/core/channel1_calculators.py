...
def _filter_by_time(data: Dict, source: str, apply_time_dt: datetime, window_days: int) -> List:
    ...
    if source == 'applist':
        ...
        fdc = data.get('params', {}).get('FDC') or {}
        if source == 'fdc_pinjaman':
            items = fdc.get('pinjaman') or []
            filtered = []
            for loan in items:
                ...
                try:
                    ...
                    continue
                # 需求1：放款日在窗口内且 <= 申请时间
                if not (cutoff <= disburse_date <= apply_time_dt):
                    continue
                # 需求1：tgl_pelaporan_data（数据上报时间）必须在申请时间之前
                # FDC 按上报时间保留数据，上报时间在申请之后的贷款不应被使用
                report_str = loan.get('tgl_pelaporan_data', '')
                if report_str:
                    try:
                        report_date = datetime.strptime(report_str, '%Y-%m-%d')
                        if report_date > apply_time_dt:
                            continue  # 上报时间在申请之后，该记录当时不可用
                    except:
                        pass
                # 需求2：如果启用了逾期条件过滤（kualitas_pinjaman/dpd/status_pinjaman=w等），
                # 还需要检查贷款到期日在申请时间之前，因为未到期贷款的逾期状态可能是脏数据
                # 此处的到期日过滤由调用方通过 cond 参数控制
                filtered.append(loan)
            return filtered
        elif source == 'fdc_inquiry':
            inquiries = fdc.get('history_inquiry') or {}
            # 从 statistic 的预聚合值构造虚拟记录，使 calc_count 能正确计数
            # key 映射: "30_hari" → 30天, "90_hari" → 90天, 等
            stat = inquiries.get('statistic') or {}
            window_key_map = {
...
def _is_overdue_cond(cond: Optional[Dict]) -> bool:
    """判断条件是否为逾期相关条件"""
    if not cond:
        return False
    for key, value in cond.items():
        # kualitas_pinjaman >= 2 表示逾期/关注
        if key == 'kualitas_pinjaman':
            return True
        # dpd_* 字段
        if key.startswith('dpd'):
            return True
        # 已结清状态（w=writeoff）
        if key == 'status_pinjaman' and value in ('w', 'W'):
            return True
    return False


def _due_date_passed(loan: Dict, apply_time_dt: datetime) -> bool:
    """检查 FDC 贷款是否已到期（到期日在申请时间之前）

    未到期的贷款即使 dpd=0 也只能说明"暂时未逾期"，
    不能说明信用良好，逾期类特征应排除未到期贷款。
    """
    due_str = loan.get('tgl_jatuh_tempo_pinjaman', '')
    if not due_str:
        # 没有到期日信息则保守放行（让其他条件过滤）
        return True
    try:
        due_date = datetime.strptime(due_str, '%Y-%m-%d')
        return due_date <= apply_time_dt
    except:
        return True
...
def calc_count(data: Dict, field_set: str, apply_time_dt: datetime,
    ...
    if cond:
        ...
        if field_set != 'fdc_inquiry':
            ...
            # 需求2：逾期类条件 → 只保留已到期贷款
            if _is_overdue_cond(cond) and field_set == 'fdc_pinjaman':
                items = [i for i in items if _due_date_passed(i, apply_time_dt)]
...
def calc_proportion(data: Dict, target_cond: Dict, apply_time_dt: datetime,
    ...
    # 需求2：逾期类条件 → 只保留已到期贷款
    if _is_overdue_cond(target_cond) and source == 'fdc_pinjaman':
        target_items = [i for i in target_items if _due_date_passed(i, apply_time_dt)]
