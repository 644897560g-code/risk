...
开发一个面向**印尼市场短期现金贷业务**的风险特征挖掘Agent系统。历史上项目按六个独立Agent串行推进；当前主线已经演进为 **FeatureOrchestrator + FeatureDevelopmentAgent + Skills** 的合并架构：

```
数据分析/知识准备(离线或简化)
  → 特征开发Agent(设计+工程+self-review)
  → 特征评估Agent(IV/PSI/覆盖率)
  → 反馈聚合
  → 特征部署Agent(打包线上服务)
```

历史六Agent职责仍可作为业务分层理解：

1. **数据分析Agent** - 分析客户申请信息、applist、FDC与贷后表现，形成业务知识
2. **特征设计Agent** - 基于业务知识和FDC变量清单设计新增特征
3. **特征工程Agent** - 根据特征设计结果开发计算代码
4. **特征工程审核Agent** - 历史审核环节，当前由self-review和post_hook校验替代
5. **特征评估Agent** - 计算IV、PSI、非缺失覆盖率，筛选优质特征
6. **特征部署Agent** - 将保留特征代码打包供风控团队部署

## 当前代码目录边界（2026-06-08归档后）

| 目录/文件 | 当前定位 | 开发注意事项 |
|-----------|----------|--------------|
| `backend/` | Web/API/Celery管理后台 | 当前工程入口之一 |
| `agents/feature_orchestrator.py` | 主流程编排 | 批量生产、评估、部署从这里走 |
| `agents/feature_development_agent.py` | 当前特征开发Agent | 合并设计+工程+self-review |
| `agents/feature_mass_producer.py` | 确定性批量特征生产 | 当前批量生产核心 |
| `agents/feature_evaluation_agent.py` | 特征评估 | IV/PSI/覆盖率与HTML报告 |
| `agents/feature_deployment_agent.py` | 部署包生成器 | 修改部署产物模板时必须同步这里 |
| `agents/template_generation_agent.py` | 模板生成 | Web端模板生成接口会调用 |
| `agents/stepwise_framework_design.py` | 三阶段设计框架 | 保留复用，不归档 |
| `agents/legacy/` | 旧版分离式Agent和历史实验 | 只用于复现/参考，新开发不要依赖 |
| `scripts/one_off/` | 一次性数据处理、debug、历史产物生成脚本 | 运行时需从仓库根目录设置 `PYTHONPATH=.` |
| `data/rule_engine_classifier.py` | 在线APP规则分类器 | APP分类子系统，不能当一次性脚本删除 |
| `data/batch_classify_new_apps.py` | 夜间未知APP批量分类 | 离线任务，仍保留在 `data/` |
| `data/APP_CLASSIFICATION_README.md` | APP分类模块边界说明 | 清理脚本前先读 |
| `outputs/deployment/v*/` | 已生成部署包快照 | 本次未归档旧版本；不要混同源码 |

**归档原则**：
- 新增业务功能优先修改当前主链路文件，不要继续扩展 `agents/legacy/`
- 历史数据抽取/分析/修复脚本放入 `scripts/one_off/`
- 在线/离线APP分类模块属于基础设施，保留在 `data/`
- 生成部署包属于产物，源码模板问题要优先修 `agents/feature_deployment_agent.py`
...
历史六Agent阶段，每个Agent都是独立的LLM模型实例，统一使用 **qwen3.6-plus** 作为底层模型。当前主线已合并为 `FeatureDevelopmentAgent` + Skills，下面表格主要用于理解历史职责边界：
...
### 历史六Agent协作流程（已被v2合并架构替代）

> 当前运行主线请以 `agents/feature_orchestrator.py`、`agents/feature_development_agent.py`、`agents/feature_mass_producer.py`、`agents/feature_evaluation_agent.py` 和 `agents/feature_deployment_agent.py` 为准。下图保留为业务职责说明，旧版分离式实现已归档到 `agents/legacy/`。
...
当前v2/v3流程已移除独立审核Agent，改为self-review、post_hook校验和IV/PSI反馈聚合。
...
| **模块边界说明** | `data/APP_CLASSIFICATION_README.md` | 说明哪些APP分类文件不能归档/删除 |
